...
export function SkuProcessor() {
  ...
  const [processedImages, setProcessedImages] = useState<{ catalog: string; images: string[] }[]>([])
  const [error, setError] = useState<string | null>(null)
  const [failedDownloads, setFailedDownloads] = useState<{ catalog: string; url: string }[]>([])
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      const successCount = results.reduce((acc, cat) => acc + cat.images.length, 0)
      if (successCount === 0) {
        setError(
  ...
  const handleRetryFailed = async () => {
    ...
    for (const { catalog, url } of failedDownloads) {
      ...
      if (imageData) {
        const processed = frameImage ? await applyFrameWithCanvas(imageData) : imageData
    ...
    if (stillFailed.length > 0) {
      setError(`${stillFailed.length} files still failed. They may require Google account access.`)
    } else {
      setError(null)
    }
  }
  ...
  const handleDownloadSingle = (imageData: string, catalogName: string, index: number) => {
    const a = document.createElement("a")
    a.href = imageData
      ...
      <div className="flex-1 container max-w-6xl mx-auto px-4 py-8">
        <div className="space-y-8">
          <section className="grid md:grid-cols-2 gap-6">
            ...
            />
            <FileUploadZone
              title="Upload Frame (Optional)"
              description="PNG image to overlay on all downloaded images"
              accept="image/png"
              onUpload={handleFrameUpload}
              file={frameFile}
              icon="image"
              subtitle={frameImage ? `Frame loaded: ${frameImage.width}x${frameImage.height}px` : undefined}
            />
          </section>
          ...
          {catalogs.length > 0 && (
            <section className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="text-lg font-semibold text-foreground">Detected Catalogs</h2>
                ...
                <div className="flex gap-3">
                  ...
                  <Button onClick={handleProcess} disabled={!canProcess || isProcessing} size="sm">
          ...
          {(isProcessing || error) && <ProcessingStatus state={processingState} progress={progress} error={error} />}
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
                        {cat.images.map((img, idx) => (
                          <div
                            ...
                            onClick={() => handleDownloadSingle(img, cat.catalog, idx)}
                          >
                            <img
              ...
              {!hasImages && (
                <div className="text-center py-8 text-muted-foreground">
                  ...
                  <p className="text-sm mt-2">
                    Make sure the Google Drive files are shared with "Anyone with the link" permission.
                  </p>
                </div>
              )}
            </section>
